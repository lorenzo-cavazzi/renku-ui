...
class NewProjectCoordinator {
  ...
  setProperty(property, value) {
    this.model.set(`input.${property}`, value);
  }
  ...
  async getVisibilities(namespace) {
    const computeVisibilities = (max) => {
      if (max === "private")
        return ["private"];
      else if (max === "protected")
        return ["private", "protected"];
      return ["private", "protected", "public"];
    };
    if (namespace.kind === "user") {
      const visibilities = computeVisibilities("public");
      this.model.setObject({
        meta: {
          namespace: {
            fetched: new Date(),
    ...
    const visibilities = computeVisibilities(group.visibility);
    this.model.setObject({
      meta: {
        namespace: {
          fetched: new Date(),
