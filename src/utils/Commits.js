...
function createDateObject(commit) {
  return {
    ...
    date: Time.parseDate(commit.authored_date),
    readableDate: Time.getReadableDate(commit.authored_date)
  };
}
...
function createCommitsObjects(commits) {
  const enhancedCommits = commits.reduce(
    (data, commit) => {
      if (!data.lastDate || !Time.isSameDay(data.lastDate, commit.authored_date)) {
        data.lastDate = commit.authored_date;
      ...
      <ListGroupItem className="commit-object">
        <Row>
          <Col xs={12} md className="commit-cut-message">
            ...
            <TimeCaption caption={"authored"} time={props.commit.authored_date} />
          </Col>
